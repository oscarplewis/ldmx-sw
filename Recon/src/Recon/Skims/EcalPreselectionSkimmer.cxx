...
void EcalPreselectionSkimmer::produce(framework::Event &event) {
  ...
  if (use_rechits_) {
    ...
    // Check if we pass preselection
    passed_preselection = (total_rec_energy < summed_det_max_) &&
                          (num_rec_hits < n_readout_hits_max_);

    if (passed_preselection) {
      ldmx_log(debug) << "Event passed rechit preselection! Total energy: "
                      << total_rec_energy << " MeV, Num hits: " << num_rec_hits;
  ...
  }  // end of rechit-based preselection
  else {
  ...
  const auto &ecal_veto{event.getObject<ldmx::EcalVetoResult>(
    ...
    event.add("EcalPreselectionDecision", passed_preselection);
  }  // end of veto-based preselection
}
}  // namespace recon
